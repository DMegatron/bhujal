...
app.post('/api/predict/predict-groundwater', async (req, res) => {
  ...
  try {
    ...
    console.log('🔗 Calling Flask service at:', `${PREDICTION_SERVICE_URL}/predict`);
...
app.get('/api/predict/health', async (req, res) => {
  try {
    const PREDICTION_SERVICE_URL = process.env.PREDICTION_SERVICE_URL || 'http://localhost:5002';
...
app.get('/api/predict/model-info', async (req, res) => {
  try {
    const PREDICTION_SERVICE_URL = process.env.PREDICTION_SERVICE_URL || 'http://localhost:5002';
